...
choose_array_element_by_weight(const uint64_t *entries, int n_entries)
{
  ...
  const or_options_t *options = get_options();
  if(options->DynamicBandwidthsParameterK > 0){
    double sk = (double)options->DynamicBandwidthsParameterK;
    //为rand_val应用动态路由算法
    double rand_val_t = (double)(rand_val % total)/total;
    rand_val_t = pow(rand_val_t, sk);
    rand_val_t = pow(2, rand_val_t);
    rand_val =(uint64_t)total*(rand_val_t - 1);
  }


  int res_idx = select_array_member_cumulative_timei(
                           entries, n_entries, total, rand_val);
  log_info(LD_CONFIG, "DynamicBandwidthsParameterK is %lf, choose node index is %d", 
          (double)options->DynamicBandwidthsParameterK, 
          res_idx);
  return res_idx;
                                           ...
                                           bandwidth_weight_rule_t rule)
{
  ...
  uint64_t *bandwidths_u64_tmp=NULL;
  ...
  /*修改开始*/
  bandwidths_u64_tmp = tor_calloc(smartlist_len(sl), sizeof(uint64_t));
  scale_array_elements_to_u64(bandwidths_u64_tmp, bandwidths_dbl,
                              smartlist_len(sl), NULL);

  int l_s = smartlist_len(sl);
  qsort(bandwidths_u64, l_s, sizeof(uint64_t), qsort_name);
...

  {
    int idx = choose_array_element_by_weight(bandwidths_u64,
                                             smartlist_len(sl));
    for(int i = 0; i < l_s; i++){
      if (bandwidths_u64_tmp[i] == bandwidths_u64[idx])
      {
        idx = i;
        break;
      }
    }
    tor_free(bandwidths_u64_tmp);
